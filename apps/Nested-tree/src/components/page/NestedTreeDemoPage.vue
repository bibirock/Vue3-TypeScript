    ...
    div.grid.grid-cols-2.gap-8.h-full
        div.value-input.border.border-slate-500
            div.flex.justify-end.p-3
                button.bg-slate-200.text-slate-800.p-2.font-bold(@click="") {{ "＋Add New Pair" }}
            div
                div.input-list.px-5.py-2.flex(v-for="(input,index) in inputValue" key=index)
                    ...
                    button.w-10.text-sm.flex.justify-center.items-center.bg-slate-300.text-slate-800(@click="") {{ " - " }}
